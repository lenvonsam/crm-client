  ...
  export default {
    ...
    methods: {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            if (this.dialogModel === 'create') {
              this.busiRelationCreate()
            } else {
              this.busiRelationUpdate()
            }
          } else {
      ...
      async busiRelationCreate () {
        try {
        let { data } = await this.apiStreamPost('/proxy/common/post', {url: 'basicData/busiRelation/create', params: {name: this.dialogObj.name, status: Number(this.dialogObj.status)}})
          if (data.returnCode === 0) {
            this.loadData()
            this.dialogShow = false
            this.$refs.bsrlForm.resetFields()
      ...
      async batchUpdate (status) {
        try {
          let { data } = await this.apiStreamPost('/proxy/common/post', {url: 'basicData/busiRelation/batchUpdate', params: {status: status, ids: this.chooseArray.map(itm => itm.id).join(',')}})
          if (data.returnCode === 0) {
            this.loadData()
      ...
      async busiRelationUpdate () {
        try {
          let { data } = await this.apiStreamPost('/proxy/common/post', {url: 'basicData/busiRelation/update', params: {name: this.dialogObj.name, status: Number(this.dialogObj.status), id: this.dialogObj.id}})
          if (data.returnCode === 0) {
            this.currentPage = 1
            this.loadData()
            this.dialogShow = false
      ...
      async busiRelationDel (id) {
        try {
          let { data } = await this.apiStreamPost('/proxy/common/del', {url: 'basicData/busiRelation/' + id})
          if (data.returnCode === 0) {
            this.currentPage = 1
            this.loadData()
