  ...
  export default {
    ...
    data () {
      return {
        ...
        searchFormItems: [
          [{label: '加工代码', model: 'id', type: 'text', placeholder: '请输入加工代码', val: ''},
          {label: '加工名称', model: 'name', type: 'text', placeholder: '请输入加工名称', val: ''}]
        ],
        tableValue: {
          tableData: [],
          hasCbx: true,
    ...
    methods: {
      ...
      async processReqSearch (params) {
        try {
          let { data } = await this.apiStreamPost('/proxy/common/post', {url: 'basicData/processReq', params: {id: params.id, name: params.name, currentPage: (this.currentPage - 1), pageSize: this.pageSize}})
          if (data.returnCode === 0) {
            this.tableValue.tableData = data.list
            this.totalPage = data.total
          } else {
      ...
      async processReqCreate () {
        try {
        let { data } = await this.apiStreamPost('/proxy/common/post', {url: 'basicData/processReq/create', params: {name: this.dialogObj.name, status: Number(this.dialogObj.status)}})
          if (data.returnCode === 0) {
            this.loadData()
            this.dialogShow = false
            this.$refs.bsrlForm.resetFields()
      ...
      async batchUpdate (status) {
        try {
          let { data } = await this.apiStreamPost('/proxy/common/post', {url: 'basicData/processReq/batchUpdate', params: {status: status, ids: this.chooseArray.map(itm => itm.id).join(',')}})
          if (data.returnCode === 0) {
            this.loadData()
      ...
      async processReqUpdate () {
        try {
          let { data } = await this.apiStreamPost('/proxy/common/post', {url: 'basicData/processReq/update', params: {name: this.dialogObj.name, status: Number(this.dialogObj.status), id: this.dialogObj.id}})
          if (data.returnCode === 0) {
            this.currentPage = 1
            this.loadData()
            this.dialogShow = false
      ...
      async processReqDel (id) {
        try {
          let { data } = await this.apiStreamPost('/proxy/common/del', {url: 'basicData/processReq/' + id})
          if (data.returnCode === 0) {
            this.currentPage = 1
            this.loadData()
      ...
      rowEdit (obj) {
        ...
        this.dialogTitle = '更新加工需求'
        this.dialogShow = true
      },
      rowDel (obj) {
        this.confirmDialog(this, '您确认要删掉本行记录吗？').then(() => {
          this.processReqDel(obj.id)
        }, () => {
          console.log('cancel')
        })
      }
