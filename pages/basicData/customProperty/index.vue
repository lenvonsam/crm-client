  ...
  export default {
    ...
    methods: {
      ...
      async customPropertyCreate () {
        try {
        let { data } = await this.apiStreamPost('/proxy/common/post', {url: 'basicData/customProperty/create', params: {name: this.dialogObj.name, status: Number(this.dialogObj.status)}})
          if (data.returnCode === 0) {
            this.loadData()
            this.dialogShow = false
            this.$refs.bsrlForm.resetFields()
      ...
      async batchUpdate (status) {
        try {
          let { data } = await this.apiStreamPost('/proxy/common/post', {url: 'basicData/customProperty/batchUpdate', params: {status: status, ids: this.chooseArray.map(itm => itm.id).join(',')}})
          if (data.returnCode === 0) {
            this.loadData()
      ...
      async customPropertyUpdate () {
        try {
          let { data } = await this.apiStreamPost('/proxy/common/post', {url: 'basicData/customProperty/update', params: {name: this.dialogObj.name, status: Number(this.dialogObj.status), id: this.dialogObj.id}})
          if (data.returnCode === 0) {
            this.currentPage = 1
            this.loadData()
            this.dialogShow = false
      ...
      async customPropertyDel (id) {
        try {
          let { data } = await this.apiStreamPost('/proxy/common/del', {url: 'basicData/customProperty/' + id})
          if (data.returnCode === 0) {
            this.currentPage = 1
            this.loadData()
