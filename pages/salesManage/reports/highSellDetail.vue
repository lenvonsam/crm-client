...
export default {
  ...
  mounted () {
    this.$nextTick(() => {
      this.searchValStr = `nyStart=${this.dataMonthStr(new Date())}&nyEnd=${this.dataMonthStr(new Date())}&orderType=1&orderColumn=高卖&nameType=0`
      this.loadData()
    })
  },
  computed: {
    ...
    searchFormItems () {
      const searchForm = [
        ...
        [{
          label: '排序', model: 'orderColumn', val: '高卖', type: 'select',
          list: [
            { label: '高卖', value: '高卖' }
          ]
        },
  ...
  methods: {
    ...
    async exportExcel () {
      try {
        ...
        const { data } = await this.apiStreamPost('/proxy/common/post', { url: 'erpReport/export/highSellDetail', params: params })
        if (!data.list) {
          this.msgShow('导出失败')
          return false
        }
    ...
    async loadData () {
      try {
        let url = `erpReport/highSellDetail?pageSize=${this.pageSize}&currentPage=${this.currentPage - 1}&uid=${this.currentUser.id}&nameType=0`
