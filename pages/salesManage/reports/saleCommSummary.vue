  ...
  methods: {
    ...
    async exportExcel () {
      try {
        ...
        const { data } = await this.apiStreamPost('/proxy/common/post', { url: 'erpReport/export/saleCommSummary', params: params })
        if (!data.list) {
          this.msgShow('导出失败')
          return false
        }
    ...
    async loadData () {
      try {
        let url = `erpReport/saleCommSummary?pageSize=${this.pageSize}&currentPage=0&uid=${this.currentUser.id}`
