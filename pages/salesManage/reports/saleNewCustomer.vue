...
export default {
  ...
  mounted () {
    this.$nextTick(() => {
      this.searchValStr = `nyStart=${this.getPreMonth()}&nyEnd=${this.dataMonthStr(new Date())}&orderType=1&orderColumn=销量合计&nameType=0`
      this.loadData()
    })
  },
  computed: {
  ...
  methods: {
    async loadData () {
      try {
        let url = `erpReport/saleNewCustomer?pageSize=${this.pageSize}&page=${this.currentPage - 1}&uid=${this.currentUser.id}&nameType=0`
