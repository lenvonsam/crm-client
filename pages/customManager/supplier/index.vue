<template lang="pug">
  .content
    ...
    .mt-5
      basic-table(:tableValue="tableValue", :currentPage="currentPage", :pageSize="pageSize", :total="totalCount", @chooseData="selectData", @pageChange="tableChange", :loading="loading", @tableRowEdit="rowEdit", @tableRowMap = "rowMap", @sort="sortHandler")
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        breadItems: ['客户管理', '采购供应商'],
        btnGroups: [{lbl: '新增客户', dataType: 'addCustm', type: 'add'}],
        searchFormItems: [
    ...
    beforeMount () {
      ...
      this.searchFormItems[1][2]['list'] = this.enableStatus
    ...
    methods: {
      ...
      groupBtnClick (type) {
        if (type == 'add') {
          this.$router.push({path: '/customManager/supplier/form?type=formalAdd'})
        }
      },
      selectData (val) {
      ...
      rowEdit (obj) {
        this.jump('/customManager/supplier/form?type=edit&id=' + obj.id)
      },
      rowMap (obj) {
