...
div
  el-table(
    ...
    :filter-change="tableFiler", 
    @sort-change="sortChange", 
    :show-summary="tableValue.summary? tableValue.summary : false", 
    :summary-method="getSummaries")
    el-table-column(v-if="tableValue.hasCbx", type="selection", width="55")    
    el-table-column(v-if="tableValue.numShow", type="index", width="50", label="序号")
  ...
  methods: {
    ...
    getSummaries (params) {
      ...
      columns.forEach((column, index) => {
        ...
        const idx = this.tableValue.tableHead.findIndex(item => {
          return item.prop === column.property && item.summary
        })
        if (idx !== -1) {
