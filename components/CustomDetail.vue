  ...
  data () {
    return {
      ...
      formItems: [
        ...
        [{lbl: '地区', key: 'region'}, {lbl: '成立日期', key: 'setUpDateStr'}],
        ...
        [{lbl: '开票地址', type: 'self', factValue: (row) => {
        ...
        }}, {lbl: '开户账号', key: 'openAcct'}],
        [{lbl: '开户银行', key: 'openBank'}, {lbl: '所处行业', key: 'industry', type: 'object', factValue: (row) => {
          let industryNameArr = []
          if (row.length > 0) {
            row.map((item) => {
        ...
        [{lbl: '月采吨位', key: 'weightPerMonth', type: 'object', factValue: (row) => { return (row)? row + '吨' : ''}}, {lbl: '高卖情况', key: 'sellHighStatus'}],
        [{lbl: '信用情况', key: 'creditStatus'}, {lbl: '年销售额', key: 'annualSales', type: 'object', factValue: (row) => { return (row)? row + '万' : ''}}],
        [{lbl: '纳税额', key: 'taxPay', type: 'object', factValue: (row) => { return (row)? row + '万' : ''}}, {lbl: '采购物资品类', key: 'procurementGoods', type:'object', factValue: (row) => {return row.map(itm => itm.name).join(',')}}],
        [{lbl: '希望我们增加提供采购的物资品类（可详细填写）', key: 'hopeAddGoods', type:'object', factValue: (row) => {return row.map(itm => itm.name).join(',')}}, {lbl: '与其他公司的合作模式（提供服务、交易规则、物资品类）', key: 'otherCooperateModel'}],
        [{lbl: '经营物资品类', key: 'dealGoods', type:'object', factValue: (row) => {return row.map(itm => itm.name).join(',')}}, {lbl: '采购用途', key: 'procurementPurpose', type:'object', factValue: (row) => {return row.map(itm => itm.name).join(',')}}],
        [{lbl: '订金需求', key: 'depositRequirement'}, {lbl: '经营用途', key: 'dealPurpose', type:'object', factValue: (row) => {return row.map(itm => itm.name).join(',')}}],
