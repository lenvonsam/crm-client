<template lang="pug">
  div
    el-upload(:action="fileUploadUrl",list-type="picture-card", :on-preview="handlePictureCardPreview",:before-upload="beforePicUpload",name="upfile", :data="{'action': action}",:file-list="picList",:on-success="uploadSuccess", :on-remove="picReomve")
      i.el-icon-plus
    ...
    watch: {
      value (newVal, oldVal) {
        if (newVal) {
          ...
          this.picList = [{name: 'fisrt.jpg', url: this.dialogImageUrl, response: {url: this.dialogImageUrl}}]
        } else {
          this.dialogImageUrl = ''
          this.picList = []
        }
